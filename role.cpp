    ...
    ui(new Ui::Role)
{
    ...
    this->setWindowIcon(QIcon("./windows_ICON.png"));
    //set width
    ...
    //load sql
    /********************************************************************************************************************************************/
    QStringList ID_NUM=actor_sql.query_ID("/_ARMER/_P_ID/string()","../Role/SQL/","default.dbxml");
    if(ID_NUM.isEmpty())
    {
    ...
    QString X=actor_sql.getDocument("../Role/SQL/","default.dbxml",text);
    ...
    if(stateflog==1)
    {
        ...
        else if(ui->treeWidget->currentItem()->childCount() > 0 || ui->treeWidget->currentItem()->childCount() == 0)
        {
            ...
            if (m_grpMenu->isEmpty())
            {
                ...
                {
                    //">" "<"  dont show
                    if(P_value[i]!="_LARGER_THAN"&&P_value[i]!="_SMALL_THAN")
                    {
...
void Role::on_action_HELP_triggered()
{
 QDesktopServices::openUrl(QUrl(QLatin1String("http://www.baidu.com")));
}

void Role::on_action_copy_triggered()
{
    on_create_new_file_clicked();
}

void Role::on_action_edit_triggered()
{
